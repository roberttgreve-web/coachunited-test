...
- ✅ **Erledigt (01.09.2026): CTA-Tracking in Analytics.** Größtenteils schon am 31.08.2026 umgesetzt (`merkliste_add`, `print_click`, `feedback_submit`, `uebung_einreichen_submit`, `generator_start`) – dieser Eintrag war nur nicht aktualisiert worden. Lücke war der Teilen-Button: `shareExerciseBar()` (Action-Leiste) und `shareExercise()` ("Übung weiterempfehlen") lösen jetzt beide `share_click` aus, mit `source: 'action_bar'` bzw. `'weiterempfehlen'` zur Unterscheidung. WhatsApp-Kanal-Link (3 Fundstellen im Code) noch nicht geprüft/angefasst – Robert wollte sich erst auf den Teilen-Button konzentrieren, könnte bei Bedarf separat nachgezogen werden.
