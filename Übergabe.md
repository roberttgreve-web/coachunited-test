...
⚠️ **Die Ursache selbst ist damit NICHT behoben, nur die Symptome.** Der Publisher pusht bei jedem Speichern/Veröffentlichen weiterhin das **komplette** Array aller veröffentlichten Übungen inklusive `grafik_url` aus seiner eigenen lokalen Datenbank. Solange diese beiden Datenbanken (Publisher lokal, `coachunited-next/public/exercises.json`) für dasselbe Feld unterschiedliche Werte haben können, kann sich das Muster jederzeit wiederholen – z. B. wenn im Publisher künftig wieder eine Übung mit Base64-Bild landet (bekanntes Muster, s. Abschnitt 14.7/21.3), oder wenn jemand eine ältere Publisher-Datenbank-Sicherung einspielt. Ein echter struktureller Fix müsste entweder (a) die Bild-Konsolidierung fest in den Publisher selbst einbauen statt sie einmalig händisch zu machen, oder (b) `push_exercises_to_github()` dazu bringen, `grafik_url` nicht zu überschreiben, wenn die Live-Version bereits eine `/images/uebungen/`-URL trägt und die Publisher-Version das nicht tut. Nicht umgesetzt, nur dokumentiert.
