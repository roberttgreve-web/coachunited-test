...
### 21.7 ⚠️ Zweiter Rückfall (01.09.2026): Publisher-Push hat wieder alle 184 Bilder zerstört – jetzt behoben, Ursache bleibt

Robert meldete: „Von fast allen Übungen fehlen die Fotos." Bestätigt per Diff gegen die Git-History: **alle 184** `grafik_url`-Werte waren wieder auf den Vor-Migrations-Zustand zurückgesetzt (156 `wp-content`-URLs → 403 durch die Vercel-Firewall aus 21.4, 28 Base64 direkt im JSON) – die komplette 21.5-Konsolidierung war weg, exakt das in 21.3 beschriebene Muster, zum zweiten Mal.

**Konkreter Auslöser dieses Mal:** Der manuelle `push_exercises_to_github()`-Aufruf vom Vormittag (Speichern-geht-nicht-live-Fix, um Übung #207 nachzuziehen, s. CoachPublisher-Übergabe Abschnitt 2.1) hat das **komplette** `public/exercises.json` mit der veralteten Publisher-Kopie überschrieben. Git-History zeigt den exakten Umschlagpunkt: Commit `5cedb2a` (27.08.) noch gut, direkt danach durch einen Publisher-Push kaputt, `06ec3db` (28.08.) repariert (183/183 gut, inkl. Firewall-Fix), dann gut bis heute – Commits `3085d49`/`9c6cac5` (heute, meine Pushes) wieder 0/184 gut.

**Fix (heute):**
- 182 der 184 `grafik_url`-Werte 1:1 aus dem letzten bekannt guten Commit `06ec3db` wiederhergestellt (nur das Feld, nicht die ganzen Objekte – andere zwischenzeitliche Edits an diesen Übungen bleiben erhalten).
- Die 2 seither neu hinzugekommenen Übungen (#183, #207) hatten ihr Bild als Base64 im JSON – dekodiert, als WebP unter `public/images/uebungen/` gespeichert, `grafik_url` gesetzt.
- Bei #183 zusätzlich den `url_slug`-bedingten Umlaut (`wählen`) im Dateinamen durch eine ASCII-Transliteration (`waehlen`) ersetzt – exakt das in 21.6 als Hypothese 1 vermutete Muster (nicht der `url_slug` selbst wurde geändert, nur der Bild-Dateiname/die Bild-URL).
- **Kritisch:** Dieselbe Korrektur wurde auch in `data/exercises.json` **im Publisher-Repo** übernommen (s. CoachPublisher-Übergabe Abschnitt 4) – ohne diesen Schritt hätte der nächste normale Speichervorgang einer veröffentlichten Übung den Fehler sofort wieder reproduziert.

✅ **Ursache behoben (01.09.2026, im CoachPublisher-Repo).** Der Publisher lädt Base64-Bilder jetzt beim Speichern sofort selbst per GitHub-API als permanente WebP-Datei nach `public/images/uebungen/` hoch (`resolve_and_upload_grafik()` in `api_save()`, plus ein zweites Sicherheitsnetz direkt in `push_exercises_to_github()`) – Base64 kann dadurch gar nicht mehr dauerhaft in der Publisher-Datenbank stehen bleiben, das Muster aus 21.3/21.7 kann sich so nicht mehr wiederholen. Details in `coachunited-publisher/Übergabe.md` Abschnitt 4.
...
**Nachtrag – eigener Fehler, zweimal nachgebessert:**
- Beim ersten Push fehlte der Spezifitäts-Fix live trotzdem: Beim Zurücksetzen der lokalen Testdaten wurde aus Versehen eine ältere Sicherungskopie zurückgespielt, die den Fix noch nicht enthielt – committet wurde dadurch die unfertige Version. Lektion: beim lokalen Testen die Original-Datei gar nicht erst mit Testdaten überschreiben, sondern in eine separate Vorschau-Datei schreiben (seitdem Standard-Vorgehen für alle weiteren lokalen Tests in dieser Session).
- Danach fiel auf, dass `.content` sitewide über `desktop.css` auf `max-width:960px` gedeckelt ist (für textlastige Seiten gedacht) – auf `/uebungen` unnötig viel Leerraum neben dem 3er-Grid. Für diese Seite auf `max-width:none` gesetzt, das Grid nutzt seitdem die volle 4fr-Spaltenbreite neben dem Filter.
...
## 34. Landingpages: dieselbe Bild-Kachel-Optik wie /uebungen, vorgefiltert (09/2026)

Die 22 Alter/Skill/Phase-Landingpages hatten bisher ein eigenes, textlastiges Kartendesign (`.exercise-card`) plus ein kleines Vorschau-Karussell (`.pg-carousel`, erst letzte Woche eingebaut, s. Abschnitt 29.9-Nachtrag „Bild-Karussell"). Beides ersetzt durch dieselbe `.uc-tile`-Kachel-Optik wie auf `/uebungen` (Bild, Titel, ungekürzte Beschreibung, Tags) – automatisch vorgefiltert nach `FILTER_KEY`/`FILTER_VALUE` der jeweiligen Seite (z. B. zeigt die G-Jugend-Seite nur G-Jugend-Übungen).

**Neue Reihenfolge:** Kachel-Grid/Slider direkt unter dem Filter, der Einleitungstext („Was macht gutes X aus?") jetzt darunter statt davor (vorher: Karussell → Text → volle Liste; jetzt: Grid/Slider → Text). Die separate „Alle X Übungen"-Zwischenüberschrift vor der alten Liste ist weg – die Trefferzahl steht bereits in der dunklen Filterleiste.

**Umsetzung:**
- `build-landing-pages.js`: `renderKarte()` (zuletzt nur noch von diesen 22 Seiten genutzt) komplett entfernt – alle 23 Seiten nutzen jetzt einheitlich `renderUebungenTile()`. `renderSliderKachel()`/`SLIDER_ANZAHL` (für das jetzt entfernte `pg-carousel`) ebenfalls entfernt. `schlank()` ergänzt `grafik_url` über `resolveGrafik()` für den client-seitigen `allExercises`-Datensatz jeder Seite (vorher fehlte das Feld dort komplett).
- Client-JS `renderEx()` je Seite auf dasselbe `uc-tile`-Markup umgestellt, bestehende Filterlogik (Sekundärfilter Skill/Phase, `applyFilters()`/`toggleOption()`) unverändert – nur die Rendering-Vorlage hat sich geändert.
...
- [x] Zweikampf — in Google Ads angelegt (2026-09-17)
...
## 38. Zwei Mobil-Regressionen behoben: Sticky-Skizze-Lücke und WhatsApp-Vorschautext (09/2026)

Zwei Fehler, die in früheren Sessions schon einmal behoben worden waren, waren durch spätere Änderungen wieder aufgetaucht.

**a) Lücke über der sticky Skizze auf der Übungsseite (Mobil):** Beim Scrollen blieb oberhalb der fixierten Übungsskizze eine leere weiße Lücke stehen, statt dass die Skizze direkt am oberen Bildschirmrand andockt. **Ursache:** `.grafik-panel` hatte `position: sticky; top: 62px`. Dieser Wert stammte aus der Zeit vor Abschnitt 33 – damals gab es noch eine separate weiße Action-Bar (Merkliste/Teilen/Drucken) direkt unter der dunklen Back-Bar, die selbst sticky bei `top: 0` war und genau 62px hoch war; die Skizze dockte direkt darunter an. Mit Abschnitt 33 wurde diese weiße Action-Bar entfernt (CTAs wanderten in die dunkle Back-Bar), aber der jetzt verwaiste `top: 62px`-Wert bei `.grafik-panel` blieb unangetastet stehen – ohne die Action-Bar davor entsteht dadurch eine 62px hohe Lücke, sobald die Skizze sticky wird. **Fix:** `top: 0`.

**b) WhatsApp-Linkvorschau zeigt wieder einen Beschreibungstext:** Abschnitt „Fix: og:description in Übungsseiten entfernt" (weiter oben) hatte das `<meta property="og:description">`-Tag beim Seitenbau komplett aus dem HTML entfernt, damit WhatsApp/Facebook nur Titel + Skizze zeigen, keinen Text. Das Tag fehlte dadurch aber vollständig im Markup – und WhatsApps Vorschau-Crawler fällt in diesem Fall auf das normale `<meta name="description">` zurück (das eigentlich nur für Google-Suchergebnisse gedacht ist, befüllt mit `seo_meta_description`), und zeigt diesen Text doch wieder unter dem Bild an. **Fix:** `build-exercise-pages.js` lässt das `og:description`-Tag jetzt im Markup stehen, aber leer (`content=""`), statt es zu entfernen – ein leeres, aber vorhandenes Tag signalisiert Crawlern „bewusst keine Beschreibung" und verhindert den Rückfall auf `<meta name="description">`. Betrifft nur `/uebung/*`-Seiten; Einheiten-, Artikel- und statische Seiten füllen `og:description` weiterhin normal (dort nie ein Problem, nie geändert).

**Lehre für beide:** Wenn ein Element entfernt wird, auf das eine andere Stelle sich implizit verlässt (Sticky-Offset, Tag-Anwesenheit als Signal), muss aktiv nach Abhängigkeiten gesucht werden – beide Regressionen sind an genau der Stelle entstanden, wo eine frühere, korrekte Lösung durch eine spätere, unabhängige Änderung unterlaufen wurde, ohne dass es zum Zeitpunkt der späteren Änderung auffiel.

## 39. Google Consent Mode v2: verlorene WhatsApp-Conversions durch Cookie-Gate (09/2026)

**Auslöser:** Robert berichtete, dass der WhatsApp-Kanal seit Start der Ad-Grants-Kampagne zuverlässig täglich 3–5 neue Abonnenten bekommt, Google Ads aber fast durchgehend 0 Conversions zeigt (Conversion-Aktion „Klick auf externen Link" → `whatsapp_click`, s. Abschnitt 29.2).

**Diagnose:** Die Verkabelung selbst war korrekt und unverändert – `trackWhatsAppClicks()` in `desktop-nav.js` feuert bei jedem Klick auf einen WhatsApp-Kanal-Link `gtag('event', 'whatsapp_click', ...)`. Der eigentliche Bruch lag in `cookie-consent.js`: `window.gtag` wurde bisher **erst nach** einer positiven Cookie-Entscheidung erzeugt (`loadGA()` lief nur im Statistik-Zweig). Klickte jemand auf den WhatsApp-Button, bevor er das Cookie-Banner beantwortet hatte – sehr typisch bei mobilem Ad-Traffic, der direkt auf den prominenten CTA tippt – lief `trackWhatsAppClicks()` ins Leere (`typeof window.gtag !== 'function'`), und das Event ging spurlos verloren, obwohl der Klick/Abo real passiert ist.

**Fix – Google Consent Mode v2** (`cookie-consent.js`):
- `gtag` existiert jetzt ab dem allerersten Seitenaufruf, nicht erst nach Einwilligung. Direkt beim Skriptstart: `gtag('consent', 'default', {ad_storage:'denied', ad_user_data:'denied', ad_personalization:'denied', analytics_storage:'denied'})` – rechtlich weiterhin einwandfrei (kein Tracking/keine Cookies ohne Zustimmung), aber das GA4-Skript selbst lädt sofort und kann anonyme, cookielose Signale senden.
- Trifft der Besucher seine Entscheidung (Banner oder Einstellungen-Modal), schaltet `gtag('consent', 'update', {...granted/denied...})` den Status um, statt wie vorher `loadGA()` überhaupt erst aufzurufen.
- Alle vier Consent-Signale (`ad_storage`, `ad_user_data`, `ad_personalization`, `analytics_storage`) hängen am selben „Statistik"-Schalter, da die Seite nur diese eine Opt-in-Kategorie hat, keine separate Marketing-Kategorie – für den GA4→Ads-Conversion-Import braucht Google auch das `ad_*`-Signal, sonst bleibt die Modellierung schwächer.
- `desktop-nav.js`: zwei veraltete Kommentare korrigiert, die noch „gtag existiert nur nach Einwilligung" behaupteten.

**Warum das hilft, ohne rechtlich problematisch zu werden:** Mit Status „denied" setzt Google **keine** Cookies und identifiziert niemanden – es sendet nur anonyme, aggregierte Signale, aus denen Google Ads einen Teil der sonst komplett unsichtbaren Conversions statistisch modellieren kann (offizieller, von Google für EU-Werbetreibende vorausgesetzter Standardweg). Das ist keine 1:1-Wiederherstellung aller verlorenen Conversions, sollte aber die Lücke zwischen realen Abos und gemessenen Conversions spürbar verkleinern.
